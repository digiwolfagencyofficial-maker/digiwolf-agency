    ...
    <div style={{ background: '#0A0A0A', minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '24px' }}>
      <div style={{ width: '100%', maxWidth: 420 }}>
        <div style={{ textAlign: 'center', marginBottom: 40 }}>
          ...
          <p style={{ fontSize: 15, color: '#C0C8D8' }}>Sign in to your Digi Wolf client portal</p>
        </div>
          ...
          <form style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
            ...
            <div>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <Link href="/forgot-password" style={{ fontSize: 13, color: '#3d74ff' }}>Forgot password?</Link>
              </div>
              <input type="password" style={input} placeholder="••••••••" value={form.password} onChange={e => setForm({...form, password: e.target.value})}/>
            </div>
            <button type="submit" style={{ background: '#0047FF', color: '#fff', padding: '14px', borderRadius: 10, fontWeight: 700, fontSize: 15, border: 'none', cursor: 'pointer', boxShadow: '0 0 24px rgba(0,71,255,0.35)', marginTop: 4 }}>Sign In →</button>
          </form>
          <div style={{ textAlign: 'center', marginTop: 24, fontSize: 14, color: '#C0C8D8' }}>
            Don't have an account? <Link href="/register" style={{ color: '#3d74ff', fontWeight: 600 }}>Register</Link>
          </div>
        </div>
